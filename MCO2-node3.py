...
query = f"""
         SELECT *
         FROM `DIMGAME` WHERE (Windows = 1 AND Linux = 1 AND Mac = 0) OR 
         (Windows = 1 AND Mac = 1 AND Linux = 0) OR 
         (Mac = 1 AND Linux = 1 AND Windows = 0);
     """
...
with displayGames:
    st.write("List of all games with 2 platforms supported")
    st.dataframe(games)
...
with addGame: # Add to the database
    with st.form("addGameForm"):
        ...
        if submitAdd:
            if not addTitle:
            ...
            elif not (windowsPlat and macPlat and not linuxPlat) or not (windowsPlat and linuxPlat and not macPlat) or not (linuxPlat and macPlat and not windowsPlat):
                st.error("Please select at least two platforms.")
            else:
                windowsVal = 0
                macVal = 0
                linuxVal = 0
                ...
                try:
                    ...
                    """
                    vals = (appIdNext, addTitle, release_date, gamePrice, windowsVal, macVal, linuxVal)
                    print("Query:", query)
                    print("Values:", vals)
